...
bool DOS_Shell::CheckConfig(char* cmd_in,char*line) {
	...
	if(line && !line[0]) {
		...
		if(val.compare(NO_SUCH_PROPERTY) != 0) WriteOut("%s\n",val.c_str());
		return true;
	}
...
//TODO Maybe support confirmation for *.* like dos does.	
	...
	char * end= (char*)strrchr(full,'\\')+1;*end=0;
...
void DOS_Shell::CMD_RENAME(char * args){
	...
	if(slash) { 
		...
		char* dummy = (char*)strrchr(dir_source,'\\');
...
static void FormatNumber(Bit32u num, char* buf) {
	...
	numg = num;
	if (numg) {
		sprintf_f(buf, "%d,%03d,%03d,%03d", numg, numm, numk, numb);
		return;
	};
	if (numm) {
		sprintf_f(buf, "%d,%03d,%03d", numm, numk, numb);
		return;
	};
	if (numk) {
		sprintf_f(buf, "%d,%03d", numk, numb);
		return;
	};
	sprintf(buf, "%d", numb);
}
...
void DOS_Shell::CMD_COPY(char * args) {
	...
	while(sources.size()) {
		...
		// cut search pattern
		char* pos = (char*)strrchr(pathSource,'\\');
		if (pos) *(pos+1) = 0;
	...
	//try {
		...
		if((arg=="/D") || (arg=="/d")) {
			if(!Drives[temp_str[0]-'A'] ) 
				Syscall_Panic("CMD_SUBST"); //targetdrive not in use
		...
		if (Drives[temp_str[0] - 'A'])
			Syscall_Panic("CMD_SUBST"); //targetdrive in use
		strcat(mountstring,temp_str);
		strcat(mountstring," ");
		...
		if ((ldp = (localDrive*)(Drives[drive])) == 0)
			Syscall_Panic("CMD_SUBST");

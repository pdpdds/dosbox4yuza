		...
		case'\t':
			{
				if (l_completion.size()) {
					...
					// get completion mask
					char *p_completion_start = (char*)strrchr(line, ' ');
					...
					char *path;
					if ((path = (char*)strrchr(line+completion_index,'\\'))) completion_index = (Bit16u)(path-line+1);
					if ((path = (char*)strrchr(line+completion_index,'/'))) completion_index = (Bit16u)(path-line+1);
					...
					if (p_completion_start) {
						...
						char* dot_pos= (char*)strrchr(mask,'.');
						char* bs_pos= (char*)strrchr(mask,'\\');
						char* fs_pos= (char*)strrchr(mask,'/');
						char* cl_pos=(char*)strrchr(mask,':');

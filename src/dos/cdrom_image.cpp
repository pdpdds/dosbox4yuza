...
bool CDROM_Interface_Image::BinaryFile::read(Bit8u* buffer, int seek, int count)
{
	...
	size_t readCount = fread(buffer, count, 1, file);
...
bool CDROM_Interface_Image::SetDevice(char* path, int forceCD)
{
	...
	sprintf(buf, "Could not load image file: %s\n", path);
...
#if defined(WIN32) || defined(SKYOS32)
static string dirname(char* file) {
	const char* sep = strrchr((const char*)file, '\\');
	if (sep == NULL)
		sep = strrchr(file, '/');
	if (sep == NULL)
		return "";
...
bool CDROM_Interface_Image::LoadCueSheet(char* cuefile)
{
	...
	while (!feof(in)) {
		// get next line
		char buf[MAX_LINE_LENGTH];
		fgets(buf, MAX_LINE_LENGTH - 1, in);

		if (!feof(in)) return false;  // probably a binary file
		istringstream line((char*)buf);
...
bool CDROM_Interface_Image::GetCueKeyword(string& keyword, istream& in)
{
	...
	for (Bitu i = 0; i < strlen(buf); i++) keyword[i] = toupper(keyword[i]);
...
bool CDROM_Interface_Image::GetCueFrame(int& frames, istream& in)
{
	...
	in >> buf;
	int min, sec, fr;
	bool success = sscanf(buf, "%d:%d:%d", &min, &sec, &fr) == 3;

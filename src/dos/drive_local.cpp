...
bool localDrive::FileOpen(DOS_File * * file,char * name,Bit32u flags) {
	...
	for (i=0;i<DOS_FILES;i++) {
		if (Files[i] && Files[i]->IsOpen() && Files[i]->GetDrive()==drive && Files[i]->IsName(name)) {
			lfp= static_cast<localFile*>(Files[i]);
			if (lfp) lfp->Flush();
		}
	}
...
bool localDrive::FileUnlink(char * name) {
	...
	if (unlink(fullname)) {
		...
		struct stat buffer;
		if(stat(fullname,&buffer) != 0) return false; // File not found.
...
again:
	...
	char* expandName = dirCache.GetExpandName(full_name);
	int result = stat(expandName, &stat_block);
	if (result !=0) 
	{
		goto again;//No symlinks and such
	}	
	if (stat_block.st_mode == 0) 
	...
	if((time=localtime(&rawtime))!=0){
		...
		//find_date = DOS_PackDate((Bit16u)(stat_block.fdate >> 9) + 1980, (Bit16u)((stat_block.fdate >> 5) & 15), (Bit16u)(stat_block.fdate & 31));
		//find_time = DOS_PackTime((Bit16u)(stat_block.ftime >> 11), (stat_block.ftime >> 5) & 63, 0);
...
bool localDrive::GetFileAttr(char * name,Bit16u * attr) {
	...
	if (stat(newname,&status)==0) {
		*attr=DOS_ATTR_ARCHIVE;
		if (status.st_mode == 0) *attr|=DOS_ATTR_DIRECTORY;
		return true;
	}
	*attr=0;
	return false; 
...
bool localDrive::TestDir(char * dir) {
	...
	if (len && (newdir[len-1]!='\\')) 
	{
		...
		if (test.st_mode != 0)	return false;
	};
...
bool localDrive::FileExists(const char* name) {
	...
	if (temp_stat.st_mode == 0) return false; 
	return true;
}
...
#else
	//20191225
	time_t _time = temp_stat.st_mtime;
	if((time=localtime(&_time))!=0) {
#endif
		//stat_block->time = DOS_PackDate((Bit16u)(temp_stat.fdate >> 9) + 1980, (Bit16u)((temp_stat.fdate >> 5) & 15), (Bit16u)(temp_stat.fdate & 31));
		//stat_block->date = DOS_PackTime((Bit16u)(temp_stat.ftime >> 11), (temp_stat.ftime >> 5) & 63, 0);
...
#else
	time_t _time = temp_stat.st_mtime;
	if((ltime=localtime(&_time))!=0) {
#endif
		//time = DOS_PackDate((Bit16u)(temp_stat.fdate >> 9) + 1980, (Bit16u)((temp_stat.fdate >> 5) & 15), (Bit16u)(temp_stat.fdate & 31));
		//date = DOS_PackTime((Bit16u)(temp_stat.ftime >> 11), (temp_stat.ftime >> 5) & 63, 0);

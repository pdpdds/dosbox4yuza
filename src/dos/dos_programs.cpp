...
public:
	...
	void Run(void) {
		...
		if (cmd->FindExist("-cd",false)) {
			...
			WriteOut(MSG_Get("PROGRAM_MOUNT_CDROMS_UNSUPPORTED"));
#else
...
// Nothing to do here.
			...
			if (failed) {
#else
			if (stat(temp_line.c_str(),&test) != 0) {
				failed = true;
				Cross::ResolveHomedir(temp_line);
				//Try again after resolving ~
				if(!stat(temp_line.c_str(),&test) != 0) failed = false;
			}
			if(failed) {
#endif
...
#if SDL_VERSION_ATLEAST(2,0,0)
	MSG_Add("PROGRAM_MOUNT_CDROMS_UNSUPPORTED","Sorry, but physical CDROMs are unsupported. Please check IMGMOUNT.\n");
#else

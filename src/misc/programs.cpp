...
void CONFIG::Run(void) {
	...
	while(presult != P_NOPARAMS) {
		...
		case P_HELP: case P_HELP2: case P_HELP3: {
			...
			Section_prop* psec = static_cast <Section_prop*>(sec);
			if (psec==NULL) {
				// failed; maybe it's the autoexec section?
				Section_line* pline = static_cast <Section_line*>(sec);
				...
				while (true) {
					...
					if (!strcasecmp(p->propname.c_str(),pvars[1].c_str())) {
						...
						if (p->Get_type()==Value::V_BOOL) {
							...
							Prop_int* pint = static_cast <Prop_int*>(p);
		...
		case P_AUTOEXEC_CLEAR: {
			Section_line* sec = static_cast <Section_line*>

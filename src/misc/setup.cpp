...
bool Value::set_hex(std::string const& in) {
	istringstream input(in.c_str());
	//20200702
	//input.flags(istream::hex);
	Bits result = INT_MIN;
	input >> result;
		...
		if (sec) {
			...
			while ((p = sec->Get_prop(i++))) {		
				...
				if (!values.empty()) {
					...
					while (it != values.end()) {
						if((*it).ToString().compare("%u") != 0) { //Hack hack hack. else we need to modify GetValues, but that one is const...
...
Section* Config::GetSectionFromProperty(char const * const prop) const{
   	for (const_it tel=sectionlist.begin(); tel!=sectionlist.end(); tel++){
		if ((*tel)->GetPropValue(prop).compare(NO_SUCH_PROPERTY) != 0) return (*tel);
	}
	return NULL;
}
...
bool Config::ParseConfigFile(char const * const configfilename){
	...
	FILE* in = fopen(configfilename, "r");
...
void Config::ParseEnv(char ** envp) {
	for(char** env=envp; *env;env++) {
		...
		if(!(*sec_name))
			continue;
		char* prop_name = (char*)strrchr(sec_name,'_');
